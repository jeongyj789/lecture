		...
		static int find(double[] data, int f, int t, int num) {
			...
			if (s == num) { 
				...
				if (cnt == 3) { 
					...
					return find(data, 0, s, 11);
				}
				if (cnt == 4) {
					return find(data, 0, s, 6);
				}
				if (cnt ==5) {
...
//				System.out.println("여기는 "+ (int) (num * (cal - 1) / cal));
...
//					System.out.println();
			...
			System.out.println(loc+" 구간 :"+(start+1)+"부터 "+end+"까지 맞음");
//			System.out.println();
		}
	...
	// 랜덤 데이터 생성	
			...
			int arr[] = {100000, 500000, 1000000, 5000000, 10000000, 50000000, 100000000};
...
//			for (int i = 1; i <= 10; i++) {
			...
			for (int n:arr) {
				...
				for (int j = 0; j < n; j++) 
				{
					...
					x *= x;		
					data[j] = 1000000 + (9000000 * x); // 점수 데이터 생성	
...
//				value_check(data, cnt_10, (cnt_20), "20%");
//				value_check(data, cnt_20, (cnt_30), "30%");
//				value_check(data, cnt_30, (data.length), "마지막");
