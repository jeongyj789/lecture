...
//			System.out.println(num*(cal-1));
	...
	public static void value_check(int[] data, int start, int end) {

		int temp = data[start]; // temp 값 기준으로 제 구간에 속해있는지 확인
//		System.out.println("기준인덱스 :" + start);
...
// 데이터 무결성 체크
			...
			value_check(data, 0, cnt_10);
			value_check(data, cnt_10, cnt_20);
			value_check(data, cnt_20, cnt_30);
			value_check(data, cnt_30, data.length);
